...
public class ZMQ {
	 ...
	 * bidirectional asynch pattern.
	 */
	public static final int DEALER = 5;
	/**
	 * Old alias for DEALER flag. Flag to specify a XREQ socket, receiving side must
	 * be a XREP.
	 * 
	 ...
	 * and consumes request-reply routing envelopes. It is the only socket type that
	 * lets you route messages to specific connections if you know their identities.
	 */
	public static final int ROUTER = 6;
	/**
	 * Old alias for ROUTER flag. Flag to specify the receiving part of a XREQ
	 * socket.
	 * 
	 ...
	 * Flag to specify the receiving part of the PUB or XPUB socket. Allows
	 */
	public static final int CLIENT = 13;
	...
	/**
	 * EVENT_CONNECTED: connection established. The EVENT_CONNECTED event triggers
	 * when a connection has been established to a remote peer. This can happen
	 * either synchronous or asynchronous. Value is the FD of the newly connected
	 * socket.
	 */
	public static final int EVENT_CONNECTED = 1;
	/**
	 * EVENT_CONNECT_DELAYED: synchronous connect failed, it's being polled. The
	 * EVENT_CONNECT_DELAYED event triggers when an immediate connection attempt is
	 * delayed and its completion is being polled for. Value has no meaning.
	 */
	public static final int EVENT_CONNECT_DELAYED = 2;
	/**
	 * EVENT_CONNECT_RETRIED: asynchronous connect / reconnection attempt. The
	 * EVENT_CONNECT_RETRIED event triggers when a connection attempt is being
	 * handled by reconnect timer. The reconnect interval's recomputed for each
	 * attempt. Value is the reconnect interval.
	 */
	public static final int EVENT_CONNECT_RETRIED = 4;

	/**
	 * EVENT_LISTENING: socket bound to an address, ready to accept connections. The
	 * EVENT_LISTENING event triggers when a socket's successfully bound to a an
	 * interface. Value is the FD of the newly bound socket.
	 */
	public static final int EVENT_LISTENING = 8;
	/**
	 * EVENT_BIND_FAILED: socket could not bind to an address. The EVENT_BIND_FAILED
	 * event triggers when a socket could not bind to a given interface. Value is
	 * the errno generated by the bind call.
	 */
	public static final int EVENT_BIND_FAILED = 16;

	/**
	 * EVENT_ACCEPTED: connection accepted to bound interface. The EVENT_ACCEPTED
	 * event triggers when a connection from a remote peer has been established with
	 * a socket's listen address. Value is the FD of the accepted socket.
	 */
	public static final int EVENT_ACCEPTED = 32;
	/**
	 * EVENT_ACCEPT_FAILED: could not accept client connection. The
	 * EVENT_ACCEPT_FAILED event triggers when a connection attempt to a socket's
	 * bound address fails. Value is the errno generated by accept.
	 */
	public static final int EVENT_ACCEPT_FAILED = 64;

	/**
	 * EVENT_CLOSED: connection closed. The EVENT_CLOSED event triggers when a
	 * connection's underlying descriptor has been closed. Value is the former FD of
	 * the for the closed socket. FD has been closed already!
	 */
	public static final int EVENT_CLOSED = 128;
	/**
	 * EVENT_CLOSE_FAILED: connection couldn't be closed. The EVENT_CLOSE_FAILED
	 * event triggers when a descriptor could not be released back to the OS.
	 * Implementation note: ONLY FOR IPC SOCKETS. Value is the errno generated by
	 * unlink.
	 */
	public static final int EVENT_CLOSE_FAILED = 256;
	/**
	 * EVENT_DISCONNECTED: broken session. The EVENT_DISCONNECTED event triggers
	 * when the stream engine (tcp and ipc specific) detects a corrupted / broken
	 * session. Value is the FD of the socket.
	 */
	public static final int EVENT_DISCONNECTED = 512;
	/**
	 * EVENT_MONITOR_STOPPED: monitor has been stopped. The EVENT_MONITOR_STOPPED
	 * event triggers when the monitor for a socket is stopped.
	 */
	public static final int EVENT_MONITOR_STOPPED = 1024;

	/**
	 * EVENT_ALL: all events known by the java binding. The EVENT_ALL constant can
	 * be used to set up a monitor for all events known by the java binding. One
	 * could add more flags
	 */
	public static final int EVENT_ALL = EVENT_CONNECTED | EVENT_CONNECT_DELAYED | EVENT_CONNECT_RETRIED
			| EVENT_LISTENING | EVENT_BIND_FAILED | EVENT_ACCEPTED | EVENT_ACCEPT_FAILED | EVENT_CLOSED
			| EVENT_CLOSE_FAILED | EVENT_DISCONNECTED | EVENT_MONITOR_STOPPED;
	...
	/**
	 * Starts the built-in 0MQ proxy in the current application thread. The proxy
	 * connects a frontend socket to a backend socket. Conceptually, data flows from
	 * frontend to backend. Depending on the socket types, replies may flow in the
	 * opposite direction. The direction is conceptual only; the proxy is fully
	 * symmetric and there is no technical difference between frontend and backend.
	 * 
	 * Before calling ZMQ.proxy() you must set any socket options, and connect or
	 * bind both frontend and backend sockets. The two conventional proxy models
	 * are:
	 * 
	 * ZMQ.proxy() runs in the current thread and returns only if/when the current
	 * context is closed.
	 * 
	 * @param frontend
	 *            ZMQ.Socket
	 ...
	 *            socket. The capture socket should be a ZMQ_PUB, ZMQ_DEALER,
	 *            ZMQ_PUSH, or ZMQ_PAIR socket.
	 * @since 3.2.2
	 */
	 ...
	 * 
	 * @param ioThreads
	 *            Number of threads to use, usually 1 is sufficient for most use
	 *            cases.
	 * @return the Context
	 */
	...
	public static class Socket implements Closeable {
		...
		/**
		 * This is an explicit "destructor". It can be called to ensure the
		 * corresponding 0MQ Socket has been disposed of.
		 */
		public void close() {
			if (closed.compareAndSet(false, true)) {
				destroy();
		 ...
		 * 'socket'. The socket type is specified at socket creation time and cannot be
		 * modified afterwards.
		 * 
		...
		/**
		 * Returns the endpoint address represented by the last bind operation. This is
		 * useful for determining the port that was assigned by the operating system in
		 * instances where the bind address contains wildcards, such as tcp://*:*.
		 *
		 ...
		 * will return immediately, with a EAGAIN error if there is no message to
		 ...
		 * 
		 * @param timeout
		 *            Timeout for receive operation in milliseconds. Default -1
		 *            (infinite)
		 */
		 ...
		 * message currently being read is not a multi-part message a value of zero
		 ...
		 * that the messages can be read or written. Check ZMQ_EVENTS option to find out
		 * whether the 0MQ socket is readable or writeable.
		 * 
		 ...
		 * If a message can be read from the socket ZMQ_POLLIN flag is set. If message
		 * can be written to the socket ZMQ_POLLOUT flag is set.
		 * 
		 ...
		 * descriptions in the man page of zmq_socket[3] for details on the exact action
		...
		/**
		 * Get the Swap. The 'ZMQ_SWAP' option shall set the disk offload (swap) size
		 * for the specified 'socket'. A socket which has 'ZMQ_SWAP' set to a non-zero
		 ...
		 * 
		 * @param swap
		 *            The value of 'ZMQ_SWAP' defines the maximum size of the swap space
		 *            in bytes.
		 */
		...
		/**
		 * Get the Affinity. The 'ZMQ_AFFINITY' option shall set the I/O thread affinity
		 * for newly created connections on the specified 'socket'.
		 * 
		 * Affinity determines which threads from the 0MQ I/O thread pool associated
		 * with the socket's _context_ shall handle newly created connections. A value
		 * of zero specifies no affinity, meaning that work shall be distributed fairly
		 ...
		 * 
		 * See also in the man page of zmq_init[3] for details on allocating the number
		 * of I/O threads for a specific _context_.
		 * 
		 * @param affinity
		 *            the affinity.
		 */
		 ...
		 * keep-alive packets for a socket connection. Possible values are -1, 0, 1. The
		 * default value -1 will skip all overrides and do the OS default.
		 * 
		 * @param optVal
		 ...
		 * 
		 * @param optVal
		 *            The value of 'ZMQ_TCP_KEEPALIVE_INTVL' defines the interval
		 *            between keepalives. Unit is OS dependant.
		 */
		 ...
		 * supported by OS). The default value -1 will skip all overrides and do the OS
		 * default.
		 * 
		 * @param optVal
		 ...
		 * multicast transports such as in the man page of zmq_pgm[7] using the
		 * specified 'socket'.
		 * 
		 * @param rate
		 */
		 ...
		 * maximum time in seconds (before version 3.0.0) or milliseconds (version 3.0.0
		 * and after) that a receiver can be absent from a multicast group before
		 * unrecoverable data loss will occur.
		 * 
		 * CAUTION: Exercise care when setting large recovery intervals as the data
		 * needed for recovery will be held in memory. For example, a 1 minute recovery
		 * interval at a data rate of 1Gbps requires a 7GB in-memory buffer. {Purpose of
		 * this Method}
		 * 
		 * @param recovery_ivl
		 */
		 ...
		 * 
		 * @param v4only
		 *            A value of true will use IPv4 sockets, while the value of false
		 ...
		 * @param plain
		 *            whether or not to use PLAIN security
		 * @since 4.0.0
		 */
		 ...
		 * 
		 * @param domain
		 *            For NULL security (the default on all tcp:// connections), ZAP
		 *            authentication only happens if you set a non-empty domain. For
		 *            PLAIN and CURVE security, ZAP requests are always made, if there
		 *            is a ZAP handler present. See http://rfc.zeromq.org/spec:27 for
		 *            more details.
		 */
		 ...
		 *
		 * <p>
		 * True means the socket will act as CURVE server, while false means the socket
		 * will not act as CURVE server, and its security role then depends on other
		 * option settings. Setting this to false shall reset the socket security to
		 * NULL. When you set this you must also set the server's secret key using the
		 * {@link #setCurveSecretKey(byte[])} function.
		 * </p>
		 *
		 ...
		 *
		 * <p>
		 * You must set this on CURVE client sockets. You can provide the key as 32
		 * binary bytes, or as a 40-character string encoded in the Z85 encoding format
		 * and terminated in a null byte. The public key must always be used with the
		 * matching secret key.
		 * </p>
		 *
		 * @param key
		 ...
		 *
		 * <p>
		 * You must set this on both CURVE client and server sockets. You can provide
		 * the key as 32 binary bytes, or as a 40-character string encoded in the Z85
		 * encoding format and terminated in a null byte.
		 * </p>
		 *
		 * @param key
		 *            to be used
		 ...
		 * You must set this on CURVE client sockets. You can provide the key as 32
		 * binary bytes, or as a 40-character string encoded in the Z85 encoding format
		 * and terminated in a null byte. This key must have been generated together
		...
		/**
		 * Sets whether socket should keep only last received/to be sent message in its
		 * inbound/outbound queue.
		 *
		 * @param conflate
		 *            A value of false is the default which means socket preserves all
		 *            messages with respect to the RECVHWM and SENDHWM options. A value
		 *            of true means only last message is kept ignoring the RECVHWM and
		 *            SENDHWM options.
		 * @since 4.0.0
		 */
		...
		/**
		 * Indicate whether socket should keep only last received/to be sent message in
		 * its inbound/outbound queue.
		 *
		 * @return true if should keep only last received/to be sent message in its
		 *         inbound/outbound queue.
		 * @since 4.0.0
		 */
		 ...
		 *
		 * @param immediate
		 *            A value of false is the default which means socket will not queue
		 *            messages to to incomplete connections. This will cause the socket
		 *            to block if there are no other connections, but will prevent
		...
		/**
		 * Sets whether the socket should automatically send an empty message when a new
		 * connection is made or accepted. This may be set on REQ, DEALER, or ROUTER
		 * sockets connected to a ROUTER socket. The application must filter such empty
		 * messages.
		 *
		 * @param isProbeRouter
		 *            if {@code true}, the socket will automatically send an empty
		 *            message when a new connection is made or accepted; if
		 *            {@code false}, no such message will be sent
		 * @since 4.0.0
		 */
		...
		/**
		 * Bind to network interface to a random port. Start listenting for new
		 * connections. The main difference from bindToRandomPort is that it allows
		 * system to pick up the right port. For this reason return type is a string and
		 * its responsibility of a caller to parse into the right integer type (e.g. for
		 * vmci transport it should be long).
		 *
		 * @param addr
		 *            the endpoint to bind to.
		 */
		...
		public native boolean sendMsg(Msg msg, int flags);

		/**
		 * Perform a zero copy send. The buffer must be allocated using
		...
		public native Object recvMsg(int flags);
		 ...
		 *
		 * @param charset
		 *            the charset of the resulting string.
		 * @return the message received, as a String; null on error.
		 */
		...
		/**
		 * Get the underlying socket handle. This is private because it is only accessed
		 * from JNI, where Java access controls are ignored.
		 * 
	...
	public static class Poller {
		...
		/**
		 * Issue a poll call. If the poller's internal timeout value has been set, use
		 * that value as timeout; otherwise, block indefinitely.
		 * 
		 ...
		 * <p>
		 * Since ZeroMQ 3.0, the timeout parameter is in <i>milliseconds</i>, but prior
		 * to this the unit was <i>microseconds</i>.
		 * 
		 * @param tout
		 *            the timeout, as per zmq_poll if -1, it will block indefinitely
		 *            until an event happens; if 0, it will return immediately;
	...
	public static class Event {
		...
		/**
		 * Get the address. For libzmq versions 3.2.x the address will be an empty
		 * string.
		 * 
		 * @return
		 */
		...
		/**
		 * Receive an event from a monitor socket. For libzmq versions 3.2.x the address
		...
		/**
		 * Receive an event from a monitor socket. Does a blocking recv. For libzmq
		 * versions 3.2.x the address will be an empty string.
		 * 
		 * @param socket
		 *            the socket
	...
	public static class Curve {
		...
		/**
		 * Returns a newly generated random keypair consisting of a public key and a
		 * secret key.
		 *
		 * <p>
